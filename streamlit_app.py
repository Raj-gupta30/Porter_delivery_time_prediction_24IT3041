...
# ------------------------------------------------------------------
    ...
    c4.metric("Test Set Size", f"{metrics['n_test']:,}")

    st.markdown("---")
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("Predicted vs Actual")
        img_path = os.path.join(EDA_DIR, "predicted_vs_actual.png")
        if os.path.exists(img_path):
            st.image(img_path, use_container_width=True)
    with col2:
        st.subheader("Training History")
        img_path = os.path.join(EDA_DIR, "training_history.png")
        if os.path.exists(img_path):
            st.image(img_path, use_container_width=True)

    st.info(
        "The model is trained with Huber loss (robust to outliers) using a "
        "feed-forward neural network (256→128→64→32→1) with batch normalization, "
        "dropout regularization, early stopping, and learning-rate scheduling."
    )
...
# ------------------------------------------------------------------
    ...
    st.subheader("Live filters (sample of cleaned training data)")
    col1, col2 = st.columns(2)
    with col1:
        hour_range = st.slider("Order hour range", 0, 23, (0, 23))
    with col2:
        cats = sorted(sample_df["store_primary_category"].unique().tolist())
        selected_cats = st.multiselect("Store categories", cats, default=cats[:8])

    filtered = sample_df[
        (sample_df["order_hour"].between(hour_range[0], hour_range[1]))
        & (sample_df["store_primary_category"].isin(selected_cats) if selected_cats else True)
    ]

    fig = px.histogram(
        filtered, x="delivery_time_minutes", nbins=50,
        title="Delivery Time Distribution (filtered)", color_discrete_sequence=["#FF6B35"]
    )
    st.plotly_chart(fig, use_container_width=True)

    fig2 = px.box(
        filtered, x="store_primary_category", y="delivery_time_minutes",
        title="Delivery Time by Category (filtered)"
    )
    fig2.update_xaxes(tickangle=45)
    st.plotly_chart(fig2, use_container_width=True)

    st.markdown("---")
    st.subheader("Pre-generated EDA plots (full training set)")
    plot_files = [
        ("target_distribution.png", "Target Distribution"),
        ("correlation_heatmap.png", "Correlation Heatmap"),
        ("delivery_time_by_hour.png", "Delivery Time by Hour"),
        ("delivery_time_by_category.png", "Delivery Time by Category"),
        ("outstanding_vs_delivery.png", "Outstanding Orders vs Delivery Time"),
    ]
    cols = st.columns(2)
    for i, (fname, title) in enumerate(plot_files):
...
# ------------------------------------------------------------------
    ...
    st.markdown(
        """
This app predicts **Porter delivery time (in minutes)** using a neural-network
regression model trained on historical delivery data.

**Pipeline:**
1. **Data cleaning** — timestamp parsing, target derivation, missing-value
   imputation, outlier capping, deduplication.
2. **Feature engineering** — cyclic time encodings, price ratios, marketplace
   load ratios (busy-partner ratio, outstanding orders per partner), and
   category cardinality reduction.
3. **EDA** — distribution, correlation, and category/time-based analyses.
4. **Modeling** — a Keras feed-forward neural network (Dense + BatchNorm +
   Dropout) trained with Huber loss, early stopping, and LR scheduling.
5. **Evaluation** — MAE, RMSE, R² on a held-out test set.
6. **Deployment** — model & preprocessing objects pickled/saved and served
   through this Streamlit app.

**Tech stack:** Python, Pandas, Scikit-learn, TensorFlow/Keras, Streamlit, Plotly.
        """
    )
